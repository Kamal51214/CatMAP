...
w, x, y, z = np.genfromtxt(r'H2_g.txt', unpack=True) 
#x=np.log10(x)
all_voltages=[]
...
#plt.cbticks(fontsize=13)
cb= fig.colorbar(axim)

ax.set_title('CO_g at -2V', fontsize=17)
ax.set_xlabel('Pressure(atm)', fontsize=14)
ax.set_ylabel('pH(unit)',fontsize=14)
cb.set_label('log10(TOF (s^-1))',fontsize=14)
#plt.cbticks(fontsize=12)
plt.show() 
